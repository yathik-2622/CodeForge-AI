...
import { useListRepositories, useConnectRepository, useAnalyzeRepository, getListRepositoriesQueryKey } from "../lib/api";
...
export default function Repositories() {
  ...
  const [analysis, setAnalysis] = useState<{ id: string; data: RepositoryAnalysis } | null>(null);
  ...
  const handleAnalyze = async (e: React.MouseEvent, repoId: string) => {
    ...
    setAnalyzing(repoId);
    try {
      const result = await analyzeRepo.mutateAsync(repoId);
      setAnalysis({ id: repoId, data: result });
    } finally {
      setAnalyzing(null);
    }
  };
  ...
  return (
    <Layout>
          ...
          {filtered.map((repo) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <p className="text-xs text-muted-foreground">{new Date(repo.createdAt).toLocaleDateString()}</p>
                <Button
                  size="sm" variant="outline"
                  className="h-7 text-xs px-2 gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
                  onClick={(e) => handleAnalyze(e, repo.id)}
                  disabled={analyzing === repo.id}
                  data-testid={`btn-analyze-${repo.id}`}
                >
                  {analyzing === repo.id ? <span className="animate-spin">⚡</span> : "✦ Analyze"}
                </Button>
              </div>
            </div>
          ))}
      ...
      {analysis && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4" onClick={() => setAnalysis(null)}>
          <div className="bg-card border border-card-border rounded-xl p-6 w-full max-w-2xl shadow-2xl max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-lg">⚡</span>
                <h2 className="font-semibold text-sm">AI Repository Analysis</h2>
              </div>
              <button onClick={() => setAnalysis(null)} className="p-1 rounded hover:bg-secondary">✕</button>
            </div>
            <div className="space-y-4">
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Summary</p>
                <p className="text-sm text-foreground leading-relaxed">{analysis.data.summary}</p>
              </div>
              {analysis.data.architecture && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Architecture</p>
                  <p className="text-sm text-foreground leading-relaxed">{analysis.data.architecture}</p>
                </div>
              )}
              {analysis.data.suggestions?.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Improvement Suggestions</p>
              ...
              <p className="text-xs text-muted-foreground">{analysis.data.filesAnalyzed} files analyzed</p>
            </div>
          </div>
        </div>
      )}
