import { useGetDashboardStats, useGetDashboardActivity, useGetAgentMetrics } from "../lib/api";
    ...
    <Layout>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={GitBranch}  label="Repositories"    value={stats?.totalRepositories ?? "—"}                                  color="bg-blue-500/10 text-blue-400" />
          <StatCard icon={Cpu}        label="Active Agents"   value={stats?.activeAgents ?? "—"}         sub="running now"              color="bg-violet-500/10 text-violet-400" />
          <StatCard icon={CheckCircle} label="Tasks Completed" value={stats?.tasksCompleted?.toLocaleString() ?? "—"} sub={`${stats?.tasksToday ?? 0} today`} color="bg-emerald-500/10 text-emerald-400" />
          <StatCard icon={ShieldAlert} label="Security Issues" value={stats?.securityIssues ?? "—"}      sub={`${stats?.criticalIssues ?? 0} critical`} color="bg-red-500/10 text-red-400" />
          <StatCard icon={Rocket}     label="Deployments"     value={stats?.successfulDeployments ?? "—"} sub="successful"             color="bg-orange-500/10 text-orange-400" />
          <StatCard icon={Code2}      label="Lines Generated" value={stats?.linesGenerated?.toLocaleString() ?? "—"}                   color="bg-blue-500/10 text-blue-400" />
        </div>
            ...
            {metrics && metrics.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={metrics} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip contentStyle={{ background: "hsl(222 47% 8%)", border: "1px solid hsl(217 32% 14%)", borderRadius: 6, fontSize: 12 }} labelStyle={{ color: "hsl(213 31% 91%)" }} />
